...
```json
{
  ...
  "schema": {
    ...
    "properties": {
      ...
      "solution_confidence": {
        "type": "number",
        "description": "0.0 to 1.0 confidence that this solution will fully resolve the caller's issue. High (≥0.85) = well-documented fix with high success rate. Medium (0.65–0.84) = common fix but may not cover all cases. Low (<0.65) = best-guess first step, escalation likely if it fails."
      },
      "citations": {
        "type": "array",
        "items": { "type": "string" },
    ...
    "required": ["voice_response", "action_type", "robot_model", "issue_category", "resolution_status", "escalation_reason", "confidence", "solution_confidence", "citations", "source_modality"]
  }
}
```
